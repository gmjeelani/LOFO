...
import { Search, Filter, MapPin, X, ArrowLeft, Send, Image as ImageIcon, CheckCircle, AlertCircle, Loader2, MessageCircle, SlidersHorizontal, Trash2, Camera, Shield, ShieldAlert, Mail, Phone, Edit2, User as UserIcon, LogOut, Facebook, Lock, PhoneCall, ClipboardList, Eye, EyeOff, Tag, RefreshCw, Link as LinkIcon, ExternalLink, Package, Calendar, HandHeart, Copy, Check, Settings, Moon, Sun, UserX, Archive, List, ChevronRight, Paperclip, Reply, Smile, MoreVertical, Zap, Bell, Info, CheckCheck } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // --- Post Flow Handlers (Abbreviated for brevity in reconstruction, but fully functional in logic) ---
  const handlePostSubmit = async (e: React.FormEvent) => {
    ...
    const newPostData: Omit<ItemPost, 'id'> = {
      ...
      contactPhone: postForm.contactPhone || user.phone || '',
      status: 'OPEN'
    };
    ...
    try {
        ...
        // Create City Alert
        if (newPostData.city) {
            let alertMsg = newPostData.type === 'LOST' 
                ? `${newPostData.itemName || newPostData.title} lost in ${newPostData.city}`
                : `${newPostData.itemName || newPostData.title} found in ${newPostData.city}`;
  ...
  const handleMarkNotificationAsRead = async (alertId: string, post?: ItemPost) => {
      ...
      try {
          ...
          await updateDoc(userRef, { readNotificationIds: arrayUnion(alertId) });
          setUser(prev => ({ ...prev, readNotificationIds: [...(prev.readNotificationIds || []), alertId] }));
          if (post) { setSelectedItem(post); setCurrentView('ITEM_DETAIL'); }
  ...
  const handleDeleteNotification = async (alertId: string) => {
      try {
          const userRef = doc(db, 'users', user.id);
          await updateDoc(userRef, { deletedNotificationIds: arrayUnion(alertId) });
          setUser(prev => ({ ...prev, deletedNotificationIds: [...(prev.deletedNotificationIds || []), alertId] }));
          showToast("Notification removed", "success");
      } catch (error) { console.error("Error deleting notification", error); }
  };

  const handleMarkAllAsRead = async () => {
      const unreadIds = notifications.filter(n => !user.readNotificationIds?.includes(n.id)).map(n => n.id);
      if (unreadIds.length === 0) return;
      try {
          const userRef = doc(db, 'users', user.id);
          await updateDoc(userRef, { readNotificationIds: arrayUnion(...unreadIds) });
          setUser(prev => ({ ...prev, readNotificationIds: [...(prev.readNotificationIds || []), ...unreadIds] }));
  ...
  const handleClearAllNotifications = async () => {
      ...
      try {
          const userRef = doc(db, 'users', user.id);
          await updateDoc(userRef, { deletedNotificationIds: arrayUnion(...allIds) });
          setUser(prev => ({ ...prev, deletedNotificationIds: [...(prev.deletedNotificationIds || []), ...allIds] }));
  ...
  const renderNotifications = () => {
      const filteredNotifications = notificationFilter === 'ALL' 
          ? notifications 
          : notifications.filter(n => !user.readNotificationIds?.includes(n.id));

      return (
          <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900">
              <div className="p-4 bg-white dark:bg-slate-900 border-b dark:border-slate-800 sticky top-0 z-10">
                  <div className="flex items-center gap-3 mb-4">
                      <button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300" /></button>
                      <h1 className="text-xl font-bold text-slate-800 dark:text-white">Notifications</h1>
                      {unreadNotificationCount > 0 && <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{unreadNotificationCount}</span>}
                  </div>
                  <div className="flex justify-between items-center">
                      <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                          {['ALL', 'UNREAD'].map(f => (
                              <button key={f} onClick={() => setNotificationFilter(f as any)} className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-colors ${notificationFilter === f ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}>{f}</button>
                          ))}
                      </div>
                      <div className="flex gap-2">
                          <button onClick={handleMarkAllAsRead} className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-emerald-600 rounded-lg transition-colors"><CheckCheck className="w-5 h-5" /></button>
                          <button onClick={handleClearAllNotifications} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 rounded-lg transition-colors"><Trash2 className="w-5 h-5" /></button>
                      </div>
                  </div>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {filteredNotifications.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full text-center p-8">
                          <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4"><Bell className="w-10 h-10 text-slate-300 dark:text-slate-600" /></div>
                          <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">All caught up!</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-500 mt-2">No notifications to show.</p>
                      </div>
                  ) : (
                      filteredNotifications.map((note) => {
                          const isRead = user.readNotificationIds?.includes(note.id);
                          return (
                              <div key={note.id} onClick={() => handleMarkNotificationAsRead(note.id, note.postId ? posts.find(p => p.id === note.postId) : undefined)} className={`group relative p-4 rounded-2xl border transition-all cursor-pointer ${isRead ? 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700' : 'bg-white dark:bg-slate-800 border-indigo-100 dark:border-indigo-900 shadow-sm'}`}>
                                  {!isRead && <span className="absolute top-4 right-4 w-2.5 h-2.5 bg-indigo-600 rounded-full animate-pulse"></span>}
                                  <div className="flex gap-4">
                                      <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${note.type === 'LOST' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>{note.type === 'LOST' ? <AlertCircle className="w-6 h-6" /> : <CheckCircle className="w-6 h-6" />}</div>
                                      <div className="flex-1 pr-4">
                                          <div className="flex justify-between items-start"><span className={`text-xs font-bold px-2 py-0.5 rounded mb-1 inline-block ${note.type === 'LOST' ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>{note.type === 'LOST' ? 'Lost Item' : 'Found Item'}</span><span className="text-[10px] text-slate-400">{new Date(note.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div>
                                          <p className={`text-sm mb-1 leading-snug ${isRead ? 'text-slate-600 dark:text-slate-400' : 'text-slate-900 dark:text-white font-semibold'}`}>{note.message}</p>
                                          <div className="flex items-center justify-between mt-2"><span className="text-[10px] text-slate-400 font-medium">{new Date(note.timestamp).toLocaleDateString()} • {note.city}</span><button onClick={(e) => { e.stopPropagation(); handleDeleteNotification(note.id); }} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100"><Trash2 className="w-4 h-4" /></button></div>
                                      </div>
                                  </div>
                              </div>
                          );
  ...
  const renderMyPosts = () => {
      ...
      return (
          <div className="flex flex-col h-full bg-white dark:bg-slate-900">
              <div className="p-4 border-b dark:border-slate-800 flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">My Posts</h1></div>
              <div className="p-4 overflow-y-auto">
                  {myPosts.length === 0 ? <div className="text-center py-10 text-slate-400">No posts yet.</div> : myPosts.map(post => (
                      <div key={post.id} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl mb-4 border border-slate-200 dark:border-slate-700">
                          <div className="flex justify-between items-start mb-2"><h3 className="font-bold text-slate-800 dark:text-white">{post.title}</h3><span className={`px-2 py-0.5 rounded text-xs font-bold ${post.type === 'LOST' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>{post.type}</span></div>
                          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{new Date(post.date).toLocaleDateString()}</p>
                          <div className="flex gap-2">
                              <button onClick={() => handleTogglePostStatus(post.id, post.status)} className={`flex-1 py-2 rounded-lg text-sm font-bold border transition-colors ${post.status === 'RESOLVED' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'}`}>{post.status === 'RESOLVED' ? 'Resolved' : 'Mark Resolved'}</button>
                              <button onClick={() => handleDeleteOwnPost(post.id)} className="px-3 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 transition-colors"><Trash2 className="w-4 h-4" /></button>
                          </div>
                      </div>
                  ))}
              </div>
  ...
  const renderInfoScreen = () => (
      <div className="flex flex-col h-full bg-white dark:bg-slate-900">
          <div className="p-4 border-b dark:border-slate-800 flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">{infoPageData.title}</h1></div>
          <div className="p-6 overflow-y-auto"><div className="prose dark:prose-invert max-w-none whitespace-pre-wrap text-slate-600 dark:text-slate-300">{infoPageData.content}</div></div>
      </div>
  );
  ...
  const renderMatchCases = () => {
    ...
    const handleMatch = async (post: ItemPost) => {
        setIsMatching(true);
        setMatchResult(null);
        try { const res = await findPotentialMatch(post, posts); setMatchResult(res); } catch (e) { showToast("Matching failed.", "error"); } finally { setIsMatching(false); }
    };
    return (
        <div className="flex flex-col h-full bg-white dark:bg-slate-900">
            <div className="p-4 border-b dark:border-slate-800 flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">Smart Match</h1></div>
            <div className="p-6 overflow-y-auto">
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl mb-6"><p className="text-indigo-800 dark:text-indigo-200 text-sm font-medium">Select one of your active posts to let our AI find potential matches.</p></div>
                {myOpenPosts.length === 0 ? <div className="text-center text-slate-400 py-8">You have no active posts.</div> : 
                    <div className="space-y-4">{myOpenPosts.map(post => (<div key={post.id} className="border dark:border-slate-700 rounded-xl p-4 flex flex-col gap-3 bg-slate-50 dark:bg-slate-800"><div className="flex justify-between"><h3 className="font-bold text-slate-800 dark:text-white">{post.title}</h3><span className="text-xs font-bold bg-slate-200 dark:bg-slate-600 px-2 py-1 rounded text-slate-600 dark:text-slate-300">{post.type}</span></div><button onClick={() => handleMatch(post)} disabled={isMatching} className="w-full py-2 bg-indigo-600 text-white rounded-lg font-bold shadow hover:bg-indigo-700 disabled:opacity-50 flex justify-center gap-2">{isMatching ? <Loader2 className="w-4 h-4 animate-spin"/> : <RefreshCw className="w-4 h-4"/>} Check Matches</button></div>))}</div>
                }
                {matchResult && <div className="mt-8 border-t dark:border-slate-800 pt-6 animate-in slide-in-from-bottom-4"><h3 className="font-bold text-lg mb-4 text-slate-800 dark:text-white">Match Analysis</h3><div className={`p-4 rounded-xl border ${matchResult.matchedId ? 'bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20' : 'bg-slate-50 border-slate-200 dark:bg-slate-800'}`}>{matchResult.matchedId ? <div><div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold mb-2"><CheckCircle className="w-5 h-5" /> Match Found! ({matchResult.confidence}% Confidence)</div><p className="text-slate-600 dark:text-slate-300 text-sm mb-4">{matchResult.reason}</p><button onClick={() => { const match = posts.find(p => p.id === matchResult.matchedId); if (match) { setSelectedItem(match); setCurrentView('ITEM_DETAIL'); } }} className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline">View Matched Item</button></div> : <div className="flex items-center gap-2 text-slate-500"><AlertCircle className="w-5 h-5" /> No strong matches found yet.</div>}</div></div>}
            </div>
        </div>
    );
  };
  ...
  const renderResolvedCases = () => {
      ...
      return (
          <div className="flex flex-col h-full bg-white dark:bg-slate-900">
              <div className="p-4 border-b dark:border-slate-800 flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">Resolved Cases</h1></div>
              <div className="p-4 overflow-y-auto">{resolved.length === 0 ? <div className="text-center py-10 text-slate-400">No resolved cases yet.</div> : resolved.map(post => (<div key={post.id} className="opacity-75 grayscale hover:grayscale-0 transition-all"><ItemCard item={post} onContact={() => { setSelectedItem(post); setCurrentView('ITEM_DETAIL'); }} /></div>))}</div>
          </div>
      );
  };
  ...
  const renderFilter = () => (
      <div className="flex flex-col h-full bg-white dark:bg-slate-900">
          <div className="p-4 border-b dark:border-slate-800 flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><X className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">Filters</h1></div><button onClick={() => { setActiveCity(''); setActiveArea(''); setActiveCategory(''); setActiveItemName(''); setCurrentView('HOME'); }} className="text-red-500 font-bold text-sm">Reset</button></div>
          <div className="p-6 space-y-6 overflow-y-auto">
              <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">City</label><select className="w-full p-3 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl" value={activeCity} onChange={e => { setActiveCity(e.target.value); setActiveArea(''); }}><option value="">All Cities</option>{cities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              {activeCity && <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Area</label><select className="w-full p-3 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl" value={activeArea} onChange={e => setActiveArea(e.target.value)}><option value="">All Areas</option>{(locations[activeCity] ? Object.keys(locations[activeCity]) : []).map(a => <option key={a} value={a}>{a}</option>)}</select></div>}
              <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Category</label><select className="w-full p-3 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl" value={activeCategory} onChange={e => { setActiveCategory(e.target.value); setActiveItemName(''); }}><option value="">All Categories</option>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
  ...
  const renderSettings = () => (
      <div className="flex flex-col h-full bg-white dark:bg-slate-900">
          <div className="p-4 border-b dark:border-slate-800 flex items-center gap-3"><button onClick={() => setCurrentView('HOME')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowLeft className="w-6 h-6 text-slate-600 dark:text-slate-300"/></button><h1 className="text-xl font-bold text-slate-800 dark:text-white">Settings</h1></div>
          <div className="p-4 space-y-6">
              <div className="space-y-2"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-2">Preferences</p><div className="bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden"><button onClick={() => setDarkMode(!darkMode)} className="w-full p-4 flex items-center justify-between hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors"><div className="flex items-center gap-3">{darkMode ? <Moon className="w-5 h-5 text-indigo-400" /> : <Sun className="w-5 h-5 text-orange-400" />}<span className="font-medium text-slate-700 dark:text-slate-200">Dark Mode</span></div><div className={`w-11 h-6 rounded-full relative transition-colors ${darkMode ? 'bg-indigo-600' : 'bg-slate-300'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${darkMode ? 'left-6' : 'left-1'}`}></div></div></button></div></div>
              <div className="space-y-2"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-2">Account</p><div className="bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden divide-y dark:divide-slate-700"><button onClick={handleChangePasswordRequest} className="w-full p-4 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-slate-700 dark:text-slate-200"><Lock className="w-5 h-5 text-slate-500" /> Change Password</button><button onClick={handleDeleteAccount} className="w-full p-4 flex items-center gap-3 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-red-600"><UserX className="w-5 h-5" /> Delete Account</button></div></div>
              <div className="space-y-2"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-2">Support</p><div className="bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden divide-y dark:divide-slate-700"><button onClick={() => handlePrivacyPolicy()} className="w-full p-4 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-slate-700 dark:text-slate-200"><Shield className="w-5 h-5 text-slate-500" /> Privacy Policy</button><button onClick={() => { setInfoPageData({ title: 'About Us', content: appSettings.aboutUs || 'LOFO.PK' }); setCurrentView('INFO_SCREEN'); }} className="w-full p-4 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors text-slate-700 dark:text-slate-200"><Info className="w-5 h-5 text-slate-500" /> About Us</button></div></div>
          </div>
      </div>
  );
        ...
        <form onSubmit={handleEmailAuth} className="w-full max-w-sm space-y-4">
            ...
            {authMode === 'LOGIN' && (
                 <div className="flex justify-between items-center text-sm">
                     ...
                     <button type="button" onClick={handleChangePasswordRequest} className="text-indigo-600 font-bold hover:underline">Forgot Password?</button>
                 </div>
            )}
  ...
  return (
    <Layout
      ...
      onMarkAsRead={(id) => handleMarkNotificationAsRead(id, notifications.find(n => n.id === id)?.postId ? posts.find(p => p.id === notifications.find(n => n.id === id)?.postId) : undefined)}
                ...
                <div className="px-4">
                     {filteredPosts.length === 0 ? (
                         <div className="text-center py-10 text-slate-400">
                             ...
                             <button onClick={() => { setSearchQuery(''); setActiveTypeFilter('ALL'); setActiveCity(''); setActiveCategory(''); setActiveArea(''); }} className="mt-4 px-6 py-2 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition-colors">Clear Filters</button>
                         </div>
                     ) : (
