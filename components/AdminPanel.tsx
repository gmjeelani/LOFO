...
import { Users, Mail, Settings, Map, FileText, BarChart, ShieldAlert, LogOut, CheckCircle, XCircle, Trash2, Save, Upload, Plus, ChevronRight, HandHeart, FileSpreadsheet, Package, Search, Image as ImageIcon, Loader2, AlertTriangle, X } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ db, onLogout, users, posts }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex font-sans transition-colors duration-200">
        {/* Sidebar */}
        <div className="w-64 bg-indigo-900 dark:bg-slate-900 text-white flex flex-col fixed h-full z-50 overflow-hidden">
            <div className="p-6 border-b border-indigo-800 dark:border-slate-800 flex items-center gap-3 shrink-0">
                <div className="w-8 h-8 bg-indigo-500 rounded-lg flex items-center justify-center font-bold">L</div>
                <h1 className="text-xl font-bold tracking-tight">LOFO Admin</h1>
            </div>
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
                {[
                    ...
                    <button key={item.id} onClick={() => setActiveTab(item.id as AdminTab)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === item.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-indigo-200 hover:bg-indigo-800 dark:hover:bg-slate-800 hover:text-white'}`}>
                        <item.icon className="w-5 h-5" /> <span className="font-medium text-sm">{item.label}</span>
                    </button>
                ))}
            </nav>
            <div className="p-4 border-t border-indigo-800 dark:border-slate-800 shrink-0">
                <button onClick={onLogout} className="w-full flex items-center gap-2 text-red-300 p-3 hover:bg-indigo-800 dark:hover:bg-slate-800 rounded-xl transition-colors"><LogOut className="w-5 h-5" /> Logout</button>
            </div>
        </div>
